...
function ProfileMenuContent({ username }: { username?: string }) {
  return (
    ...
    async function fetchNotifications() {
      ...
      try {
        const res = await fetch(
          `/api/notifications?userId=${encodeURIComponent(session.user.id)}`,
        );
        if (res.ok) {
          ...
          <div className="flex items-center gap-4">
            {isPending ? (
              ...
              <div className="flex items-center gap-4">
                ...
                >
                  <PopoverTrigger className="relative cursor-pointer hover:bg-accent hover:text-accent-foreground h-9 w-9 inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors">
